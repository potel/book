...
\xi=\frac{\hbar v_F}{\pi E_{corr}}\approx 20 \, \text{fm},
 \end{equation*}
in keeping with the fact that in $^{11}$Li, $(v_F/c)\approx 0.16$ and $E_{corr}\approx0.5$ MeV (see App. \ref{App6H}). Consequently, 
...
\kappa_1=\frac{-5V_1}{AR^2}.
 \end{equation}
 where $V_1=25$ MeV is the symmetry potential energy.
...
\end{equation}
is the ``standard'' self consistent dipole strength\footnote{cf. \cite{Bohr:75}.}. The screening factor $s=(\kappa_1/\kappa_1^0)=0.043$ is very close in magnitude to the ratio $r(=0.048)$ and has a similar physical origin. It is of notice that $(V_1)_{scr}=sV_1\approx 1$ MeV. Making use of (\ref{eq2.6.14})  one obtains,
\begin{equation*}
...
The associated particle--vibration coupling\footnote{cf. e.g. \cite{Brink:05} Eq. (8.42) p.189.} is $\Lambda= \left(\partial W(E)/\partial E|_{\hbar\omega_{pygmy}}\right)^{-1/2}$. Note the use in what follows of a dimensionless dipole single--particle field $F'=F/R(^{11}\text{Li})$). This is in keeping with the fact that one wants to obtain a quantity with energy dimensions ($[\Lambda]=$ MeV), and that $\kappa_1$ has been introduced through the Hamiltonian $H_D$ with the self consistent value normalized in terms of $R^2(^{11}$Li). 
 One then obtains
\begin{equation*}
\begin{split}
\Lambda^2&=\left\{2\hbar \omega_{pygmy}\frac{2\times 0.08(\frac{\hbar^2A}{2M})/R^2}{\left[(\tilde\epsilon_{p_{1/2}}-\tilde\epsilon_{s_{1/2}})^2-(\hbar\omega_{pygmy})^2\right]^2}\right\}^{-1},\\
&=\left\{2\text{MeV}\frac{0.16(\hbar^2A/2M)(1/4.6^2)\,\text{fm}^2}{\left[(0.45\text{ MeV})^2-(1\text{MeV})^2\right]^2}\right\}^{-1},\\
&=\left(\frac{3.3\,\text{MeV}^2}{(0.8)^2\,\text{MeV}^4}\right)^{-1}\approx 0.24\,\text{MeV}^2,
\end{split}
\end{equation*}   
 leading to $\Lambda\approx 0.5\,\text{MeV}$. The value of the induced interaction matrix elements is then given by (Fig. \ref{pigmy}),
...
M_{ind}=\frac{2\Lambda^2}{DEN}\approx-\frac{2\Lambda^2}{\hbar\omega_{pygmy}}\approx-0.5\,\text{MeV},
 ...
 \begin{align*}
e^2\rightarrow \Lambda R_0 (^{11}\text{Li})=0.5\, \text{MeV}\times 2.7 \,\text{fm};\quad a_0\rightarrow d=4\,\text{fm};\quad R\rightarrow R_{eff} (^{11}\text{Li})=4.83\,\text{fm}.
 \end{align*} 
 That is,
  \begin{align*}
...
(S_{2n})_{exp}\approx 0.380 \,\text{MeV}.
 ...
 \begin{align*}
 \tilde\epsilon_{s_{1/2}}&=0.15\, \text{MeV},\\
  \tilde\epsilon_{p_{1/2}}&=0.60\, \text{MeV},\\
  V_1&=25\, \text{MeV},
 \end{align*}
and 
  \begin{align*}
...
(V_1)_{scr}=sV_1=0.042\times 25\, \text{MeV}\approx1\,\text{MeV}.
  \end{align*}
The fact that $r$ and $s$ coincide within numerical approximations is in keeping with the fact that both quantities are closely related to the overlap\footnote{One ca equivalently use $\left(R_0/R_{eff}\right)^3\approx\left( 2.7\text{ fm}/(4.8\text{ fm})\right)^3\approx0.18$.}
  \begin{align*}
...
Historically one can distinguish
\section[Phase transition and fluctuations]{Phase transition and fluctuations\footnote{See e.g. \cite{Anderson:84,Anderson:64b,Anderson:76}. For a short overview see \cite{Brink:05} Ch. 1, p. 27. Amore detailed account of the phenomenon is done in Ch. 6.}}\label{C2AppE}
...
\begin{figure}
 ...
 \caption{(a) the measured specific heat is shown for a variety of Sn--particles. The figure is taken from \cite{Tsuboi:77}. The results of the static path approximation with quadratic (RPA--like) corrections  are shown (after \cite{Lauritzen:93}).}\label{fig2.E.2}
 \end{figure}
