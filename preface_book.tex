...
In addition to the quantum numbers $\lambda$ and $\mu$ one can characterize nuclear excitations by additional quantum numbers such as parity $\pi$, isospin $\tau$ and spin $\sigma$. Furthermore one can assign a particle (baryon or transfer) quantum number $\beta$. For a nucleon moving above the Fermi surface one has $\beta=\pm1,$ while for a hole in the Fermi sea $\beta=-1$. For bosonic excitations $\beta=0$ for a mode associated with e.g. surface oscillation, which can also be viewed as a correlated particle-hole excitation (within this context see Fig. \ref{fig1.0.7}) In particular, the low-lying quadrupole vibrations of even-even nuclei have quantum numbers $\lambda^\pi=2^+$, $\tau=0$ (protons and neutrons oscillate in phase) and $\sigma=0$ (no spin-flips in the excitation).
